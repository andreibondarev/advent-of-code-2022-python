...
with open('./input.txt') as f:
  ...
  for line in islice(f, 10, None):
    ...
    queue = deque(origin_array)

    for crate in range(int(quantity)):
      # Remove a crate off the "top" / beginning of array
      crate_in_flight = queue.popleft()

      # Drop a crate at the "top" / beginning of array
      destination_array.insert(0, crate_in_flight)

      # Re-assign origin_array to new modified stack of crates
      crates[origin_idx] = list(queue)
